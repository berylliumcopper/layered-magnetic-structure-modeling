...
constexpr double J_intra{ 13.49 * 4.0/*2.6*/ };
...
template<int p>
Result_M_C<p> M_C<p>::run(ofstream& os)
{
...
void sys_run(const char* ofile, const char* ofile_M_H, double temperature0)
{
	...
	{//
		...
		do {
			...
			{
				double rate = 1.0;
				M_C<N> au{ 65536,2048,temperature,0.5,0.05 * mag_i,ss,mm,os,rate };
				mm = au.run(os);
				for (int i = 0; i < N; i++)
				{
					theta_s.at(i) = acos(mm.mag_z.at(i) / sqrt(mm.mag_x.at(i) * mm.mag_x.at(i) + mm.mag_y.at(i) * mm.mag_y.at(i) + mm.mag_z.at(i) * mm.mag_z.at(i)));
